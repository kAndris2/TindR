...
class WelcomePage extends Component {
    ...
    doLogin(email, password) {
      axios.post("https://"+process.env.REACT_APP_IP+":8443/api/login", {
        email: email,
        password: password
      }).then(response => {
    ...
    async isValidEmail(email) {
      let temp = undefined;
      await axios.get("https://"+process.env.REACT_APP_IP+":8443/api/valid_email/" + email)
      .then(response => {
    ...
    async requestPin(number){
      await axios.post("https://"+process.env.REACT_APP_IP+":8443/api/getpin",{
    ...
    async validatePin(pinnumber){
      ...
      await mySwal.fire({
        ...
        preConfirm: (usuallytrue) => {
          return axios.post("https://"+process.env.REACT_APP_IP+":8443/api/validatecode",{
            appkey:this.state.appkey,
            apikey:this.state.apikey,
            phone:pinnumber,
            code:pin
    ...
    async doRegistration(file) {
      ...
      await axios.post("https://"+process.env.REACT_APP_IP+":8443/api/register", {
        name: userName,
        email: email,
